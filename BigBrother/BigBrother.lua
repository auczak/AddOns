...
local elixirs = {
	...
	SpellName(11406),-- 11406 Elixir of Demonslaying
...
---==============================================
function addon:EPGPConsumableCheck(only_warn)
  local totalFlaskCount = 0
  local totalElixirCount = 0
  local numElixirs = 0
  local MissingFlaskList={}
  local MissingElixirList={}
  local MissingFoodList={}
  local epgp_penalty = EPGP:GetModule("EPGP_Cache"):GetNoConsumablesPenalty()
  ...
  if self.db.profile.CheckElixirs then
	...
	if next(MissingFlaskList) == nil then
	  EPGP:GetModule("EPGP_Backend"):Report(L["Everyone has a flask or an elixir, good job!"])
	else
		if only_warn then
		  EPGP:GetModule("EPGP_Backend"):Report(L["WARNING - use flask or elixir - otherwise penalty of %d will be applied to following players: %s"], epgp_penalty, table.concat(MissingFlaskList, ", "))
		else
		  EPGP:GetModule("EPGP_Backend"):Report(L["Penalty for missing flask or elixir:"])
		  for i, v in ipairs(MissingFlaskList) do
			EPGP:GetModule("EPGP_Backend"):AddEP2Member(MissingFlaskList[i], epgp_penalty)
		  end
		end
	end
  end
  ...
  if self.db.profile.CheckFood then
    ...
    if next(MissingFoodList) == nil then
	  EPGP:GetModule("EPGP_Backend"):Report(L["Everyone has food buff, good job!"])
	else
		if only_warn then
		  EPGP:GetModule("EPGP_Backend"):Report(L["WARNING - use food buff - otherwise penalty of %d will be applied to following players: %s"], epgp_penalty, table.concat(MissingFoodList, ", "))
		else
		  EPGP:GetModule("EPGP_Backend"):Report(L["Penalty for missing food buff:"])
		  for i, v in ipairs(MissingFoodList) do
			EPGP:GetModule("EPGP_Backend"):AddEP2Member(MissingFoodList[i], epgp_penalty)
		  end
		end
	end
  end  
