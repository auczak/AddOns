...
L:RegisterTranslations("zhCN", function() return {
	...
	["Search Format String"] = "搜索格式化字串",
	["Supplied pattern is a format string instead of a pattern"] = "允许搜索带格式的字串，替代不带格式的字串。",
	["<string>"] = "<字符串>",
	...
	["Forward"] = "上条",
    ["ForwardMessage"] = "上条信息",
    ["Forward the message to a chat channel."] = "上次输入聊天频道的信息。",
    ["Output Channel"] = "输出频道",
    ["Channel to send output text to."] = "输出文本信息的频道。",
    ["Channel Data"] = "频道数据",
    ["Extra data for WHISPER (target) and CHANNEL (channel name or num)"] = "你密语(目标)和频道(频道名称或者数字)的其他数据。",
	["Output Message Only"] = "仅输出信息";
	["Only output the message portion of the chat text, leave out the channel, and playername etc."] = "仅输出聊天文字部分，不输出频道名，玩家名等信息。"
} end)
...
Prat_CustomFilters.revision = tonumber(string.sub("$Revision: 63496 $", 12, -3))
...
function Prat_CustomFilters:Prat_PostAddMessage(message, frame, event, text, r, g, b, id)
	...
	if message.CF_SINK then
		if self.db.profile.outputmessageonly then
		    self:Pour(message.MESSAGE, r, g, b)
		else
...
function Prat_CustomFilters:GetDisableReplace(p)
    return not(p.block or p.tosink)
end
