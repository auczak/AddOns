...
L[LIB.NEWLOCALE](L, "zhCN", function() return {
	...
	["AutoLoD"] = "自动按需启动",
    ["Simulate LoD like behaviour. Remember which modules are currently being used, and only load those."] = "模拟按需启动，自动记录使用的模块，并在随后仅加载这些模块。",
    ["Reload"] = "重新载入",
...
Prat.revision = tonumber(string.sub("$Revision: 63496 $", 12, -3))
...
function Prat:OnInitialize()
	...
	-- Compat Layer Start
	if not self.ScheduleEvent then
		self.ScheduleEvent = function(source, name, func, time, ...)
			PRAT_LIBRARY(LIB.TIMERS):AddTimer(name, time, func, ...)
		end
	end

	if not self.Hook then
		self.Hook = function(a, b, c)
			self:AddHook(a, b)
		end
	end
...
function Prat:RegisterModule(name, module)
    ...
    if module.db then
        DBG_MODULE("RegisterModule - DUPLICATE DB REGISTRATION", module.name)
