...
if (not XPerlColourTable) then
	XPerlColourTable = setmetatable({},{
		__index = function(self, class)
			local c = RAID_CLASS_COLORS[class] or {r = 0, g = 0, b = 0}
...
function XPerl_MTRosterChanged()
	...
	if (MainTankCount == 0 or conf.UseCTRATargets == 0) then
		-- If no defined tanks, then make a list from the warriors in raid
		MainTanks = {}
		MainTankCount = 0

		if (conf.NoAutoList == 0 and not inBattlegrounds()) then
			for i = 1,GetNumRaidMembers() do
...
-- OnEvent
function XPerl_OnEvent(self, event, a)
	Events[event](self, a)
end
...
function XPerl_EnableDisable()
	...
	if (XPerlConfigHelper and conf.RaidHelper == 1 and GetNumRaidMembers() > 0) then
		...
		if (XPerlConfigHelper and conf.RaidHelper == 0 and CT_RAMenu_Options and CT_RA_UpdateVisibility) then
